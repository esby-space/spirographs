...
The point of tangency and the two centers are no longer collinear, so the equation $(Acos(\alpha), Bsin(\alpha)) - r(cos(\alpha), sin(\alpha))$ no longer describes the center of the rotating circle. We'll need something more sophisticated. To find the offset of $M_1$ from $C_1$, let's find the slope of $\overline{CM_1}$.

To find this slope, notice how that it's perpendicular to the line tangent with the ellipse at point $C_1$. And finding the slope of the tangent line is nothing a little calculus (and our favorite trig identities) can't handle!

<img src="/spirographs/assets/ellipse4.jpg" />
